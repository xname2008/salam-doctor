...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.uniqueItems = galleryItems;
    this.uniqueLength = galleryItems.length;
  ...
  getClickedItemIndex(clientX, clientY) {
    if (!this.medias || !this.medias.length || !this.viewport) {
      return 0;
    }
    ...
    const worldX = nx * (this.viewport.width / 2);
    const worldY = ny * (this.viewport.height / 2);
    ...
    this.medias.forEach(media => {
      ...
      if (dx > halfW * 1.15 || dy > halfH * 1.25) return;
      const score = dx + dy * 0.35;
      if (score < bestScore) {
        bestScore = score;
        bestIndex = media.index % this.uniqueLength;
      }
    });
    ...
    return ((raw % this.uniqueLength) + this.uniqueLength) % this.uniqueLength;
  }
  onTouchDown(e) {
    if (e.target?.closest?.('.cg-lightbox')) return;
    this.isDown = true;
    this.dragDistance = 0;
  ...
  onTouchUp(e) {
    ...
    if (moved <= 10 && this.onSelect) {
      const index = this.getClickedItemIndex(pointer.x, pointer.y);
      this.onSelect(index, this.uniqueItems[index]);
    }
  }
  onWheel(e) {
    if (e.target?.closest?.('.cg-lightbox')) return;
...
export default function CircularGallery({
  ...
  const lightboxItems = items && items.length ? items : [];
  const isLightboxOpen = lightboxIndex !== null && lightboxItems.length > 0;
  const activeItem = isLightboxOpen ? lightboxItems[lightboxIndex] : null;

  const closeLightbox = useCallback(() => setLightboxIndex(null), []);
  const showAt = useCallback(
    next => {
      if (!lightboxItems.length) return;
      const wrapped = ((next % lightboxItems.length) + lightboxItems.length) % lightboxItems.length;
  ...
  }, [isLightboxOpen, lightboxIndex, closeLightbox, showAt]);

  const onLightboxPointerDown = e => {
    const stage = e.currentTarget;
    stage.dataset.swipeX = String(e.clientX);
    stage.dataset.swipeY = String(e.clientY);
    stage.setPointerCapture?.(e.pointerId);
  };

  const onLightboxPointerUp = e => {
    const stage = e.currentTarget;
    const startX = Number(stage.dataset.swipeX || 0);
    const startY = Number(stage.dataset.swipeY || 0);
    const dx = e.clientX - startX;
    const dy = e.clientY - startY;
    if (Math.abs(dx) < 48 || Math.abs(dx) < Math.abs(dy)) return;
    if (dx > 0) showAt(lightboxIndex - 1);
    else showAt(lightboxIndex + 1);
  };

  return (
    <>
      <div
        className="cg-webgl-gallery"
        ref={containerRef}
        tabIndex={0}
        role="region"
        aria-label="Circular image gallery. Click an image to enlarge. Use left and right arrow keys to navigate."
      />

      {isLightboxOpen && activeItem ? (
        <div
          className="cg-lightbox is-open"
          role="dialog"
          aria-modal="true"
          aria-label={activeItem.text || 'Enlarged image'}
          onClick={e => {
            if (e.target.classList.contains('cg-lightbox')) closeLightbox();
          }}
        >
          <button
            type="button"
          ...
          {lightboxItems.length > 1 ? (
            <>
              <button
                type="button"
                className="cg-lightbox__nav cg-lightbox__nav--prev"
                aria-label="Previous image"
                onClick={() => showAt(lightboxIndex - 1)}
              >
                ‹
              </button>
              <button
                type="button"
                className="cg-lightbox__nav cg-lightbox__nav--next"
          ...
          <div
            className="cg-lightbox__stage"
            onPointerDown={onLightboxPointerDown}
            onPointerUp={onLightboxPointerUp}
          >
            <img
              className="cg-lightbox__img"
              src={activeItem.image}
              alt={activeItem.text || ''}
              draggable={false}
            />
            {activeItem.text ? <p className="cg-lightbox__caption">{activeItem.text}</p> : null}
            {lightboxItems.length > 1 ? (
              <p className="cg-lightbox__counter">
