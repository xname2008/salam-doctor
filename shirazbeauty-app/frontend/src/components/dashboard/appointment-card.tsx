...
export function AppointmentCard({
  ...
  return (
    <div className="rounded-2xl border border-azure/15 bg-white p-5 shadow-soft-sm transition-shadow hover:shadow-soft">
      ...
      <dl className="mt-5 grid gap-3 text-xs text-ink-muted sm:grid-cols-2">
        <div className="flex items-center gap-2">
          <CalendarDays className="size-3.5 shrink-0 text-azure" />
          <span>{formatAppointmentDate(appointment.jalali_date)}</span>
        </div>
        <div className="flex items-center gap-2">
          <Clock className="size-3.5 shrink-0 text-azure" />
