...
export function ClinicAppointmentsBoard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-rose/15 bg-white p-4 shadow-soft-sm">
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as TabFilter)}>
        ...
        <TabsContent value={tab} className="mt-5">
          {visibleRows.length === 0 ? (
            ...
            <>
              <div className="hidden overflow-hidden rounded-3xl border border-rose/15 bg-white shadow-soft md:block">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {visibleRows.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>
                          <span className="inline-flex rounded-full bg-rose-tint px-3 py-1 text-sm font-bold text-rose num-fa">
              ...
              <ul className="grid gap-3 md:hidden">
                {visibleRows.map((row) => (
                  <li
                    key={row.id}
                    className="rounded-3xl border border-rose/15 bg-white p-4 shadow-soft-sm"
                  >
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-ink-muted">
                      ...
                      <span className="font-bold text-rose num-fa">
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <a
          href={`tel:${mobile}`}
          dir="ltr"
          className="mt-0.5 inline-flex items-center gap-1 text-start text-[11px] text-ink-muted transition-colors hover:text-rose num-fa"
        >
...
function StatPill({
  ...
  const tones = {
    ...
    azure: "from-rose-tint to-white text-rose",
  };
  return (
    <div
      className={cn(
        "rounded-2xl border border-rose/10 bg-gradient-to-l p-4 shadow-soft-sm",
        tones[tone],
      )}
    >
