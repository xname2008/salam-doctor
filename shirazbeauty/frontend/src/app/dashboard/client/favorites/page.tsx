    ...
    <>
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        {SAVED_CLINICS.map((clinic) => (
          <article
            key={clinic.id}
            className="group flex flex-col rounded-2xl border border-border bg-white p-5 shadow-soft-sm transition-all duration-300 hover:-translate-y-1 hover:border-rose/30 hover:shadow-soft"
          >
            <div className="flex items-start justify-between gap-3">
              <h2 className="text-sm font-bold text-ink transition-colors group-hover:text-rose">
                {clinic.name}
              </h2>
              <button
                type="button"
                aria-label={`حذف ${clinic.name} از علاقه‌مندی‌ها`}
                className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-rose-tint text-rose transition-colors hover:bg-destructive/10 hover:text-destructive"
              >
