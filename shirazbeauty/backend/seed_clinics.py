...
Inside the API container (copy the workbook first or mount the data volume):
    ...
    docker cp "/path/to/Book2 (7).xlsx" shirazbeauty-api:/tmp/book.xlsx
    docker exec shirazbeauty-api python seed_clinics.py --file /tmp/book.xlsx
"""
...
DEFAULT_EXCEL_PATHS = [
    Path(__file__).resolve().parent.parent / "data/uploads/Book2 (7).xlsx",
...
def main() -> int:
    ...
    parser.add_argument(
        "--file",
        dest="excel_file",
        help="Path to Book2 Excel workbook (defaults to data/uploads/).",
    )
