  ...
  <div class="container topbar-row">
    <a href="/index.html" class="brand" aria-label="سلام دکتر — صفحه اصلی">
      <span class="logo logo-heart-beat" aria-hidden="true">
        <img src="/images/logo-heart.webp" alt="لوگو سلام دکتر" width="40" height="40"
...
<nav id="mobile-drawer" class="mobile-drawer" aria-hidden="true">
  <div class="drawer-head">
    <span class="drawer-title"><span class="logo logo-heart-beat drawer-logo" aria-hidden="true"><img src="/images/logo-heart.webp" alt="" width="30" height="30" onerror="this.onerror=null;this.src='/images/logo.svg'"></span>سلام دکتر</span>
    ...
    <div class="footer-grid">
      <div class="footer-col">
        ...
        <div class="footer-links">
          ...
          <span>شیراز، ستارخان، بین فرعی ۴ و ۶</span>
...
function renderArticlePage(article) {
  ...
  const jsonLd = {
    ...
    publisher: {
      '@type': 'Organization',
      name: 'سلام دکتر',
      logo: { '@type': 'ImageObject', url: SITE_BASE + '/images/logo.svg' },
    },
  ...
  return [
    ...
    '<link rel="stylesheet" href="/assets/css/shared.css?v=20260619d">',
    ...
    '}).catch(function(){});});',
    '</script>',
    '<script src="/assets/js/shared.js?v=20260619d" defer></script>',
    '</body>',
    '</html>',
  ].join('\n');
}
...
const FEATURED_SLOT_DEFAULTS = [
  { rank: 1, id: NAHAL_FEATURED_ID, name: 'کلینیک نهال', tagline: 'کلینیک کاشت مو،پوست،لیزر و زیبایی', image: 'images/nahal.webp?v=20260614', link: 'profile.html?id=114', badge: 'ویژه' },
  ...
  try {
    ...
    for (const slot of FEATURED_SLOT_DEFAULTS) {
      ...
      const existing = db.prepare(`
        SELECT * FROM featured_clinics
        WHERE rank = ? AND active = 1 AND image IS NOT NULL
          AND TRIM(image) != '' AND image NOT LIKE '%placeholder%'
        ORDER BY CASE WHEN id = ? THEN 0 ELSE 1 END
        LIMIT 1
      `).get(slot.rank, slot.id);

      if (existing && featuredHasImage(existing) && normalizeFeaturedLink(existing.link)) {
        continue;
      }
...
function handleGetFeatured(res) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!existing || row.id === NAHAL_FEATURED_ID) {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO articles (
        ...
        @id, @slug, @title, @summary, @cover_image, @body_html, @category, @author,
        @status, 0, @created_at, @updated_at, @published_at
      )
    `).run({
      id,
      slug,
...
io.on('connection', (socket) => {
  ...
  socket.emit('chat:welcome', {
    from: 'admin',
    text: 'سلام! چطور می‌تونم راهنماییتون کنم؟',
    timestamp: nowIso(),
  });
...
async function start() {
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log('Admin: /api/admin/featured  /api/admin/ads  /api/admin/articles  /api/admin/upload  /api/admin/reports/*');
